...
void FCFS(sch * list, int v, int d){
	...
	while(new_thread.head_thread != NULL){
		//get thread				
		if(v == 1){
			verbos(new_thread.head_thread,next_arrival,new_thread.head_thread->proc_Id,-1);
		}
		...
		while(ready.head_thread != NULL ){
			...
			if(v == 1){
				verbos(ready.head_thread,time,ready.head_thread->proc_Id,block_flag);
			}
			...
			if(block_time <= time && block_time != 0){
				...
				if(blocked.head_thread->CPU_burst->next == NULL){							
					...
					if(v == 1){
						verbos(blocked.head_thread,block_time,blocked.head_thread->proc_Id,0);
					}
				}else{
					if(v == 1){
						verbos(blocked.head_thread,block_time,blocked.head_thread->proc_Id,-1);
					}							
			...
			while(ready.head_thread != NULL && ready.head_thread->CPU_burst != NULL && block_flag != 1){
				...
				//do the CPU burst and reset it to 0
				...
				if(ready.head_thread->CPU_burst->IO_time != 0){					
					...
					block_flag = 1;
					if(v == 1){
						verbos(ready.head_thread,time,ready.head_thread->proc_Id,1);
					}
			...
			if(blocked.head_thread  != NULL && ready.head_thread != NULL){
				...
				if(block_time > next_arrival && next_arrival != -1){
					...
					time = block_time;								
					if(v == 1){
						verbos(blocked.head_thread,time,blocked.head_thread->proc_Id,-1);
					}
					remove_blocked(&ready,&blocked,&block_time, time);					
				}
			}
			...
			if(ready.head_thread != NULL && ready.head_thread->CPU_burst == NULL){
				...
				if(v == 1){
					verbos(ready.head_thread,time,ready.head_thread->proc_Id,0);
				}
				add_done(&ready,&done);
				if(ready.head_thread != NULL){
					current_proc = ready.head_thread->proc_Id;
	...
	print_result(time,(avg_turnround/counter),CPU_time);
	if(d == 1){
		detail(done.head_thread);
	}
